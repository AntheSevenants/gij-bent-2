...
ds <- dataset(df=df,
              ...
              other_columns=c("distance_from_antwerp", "gender", "is_reply"))
...
# Export
write.csv(coefficients_with_labels, "gij_bent_coefficients.csv",
          row.names=FALSE)
...
write.csv(model_meta$as.data.frame(), "model_meta.csv", row.names=FALSE)
