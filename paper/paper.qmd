...
We see in @fig-region-violin that, while they are few, innovators in non-Brabant dialect areas *do* exist. One might wonder what the profile of these rare innovators is. Perhaps they they live close to the border in their specific dialect regions, and are therefore "atypically" innovative for their region. To answer this question, we built a Generalised Additive Model or GAM [@hastie_generalized_1987]. A GAM can fit smooth curves to "wavy" data. If extrapolated to two dimensions, it allows one to create "weatherman-like" heatmaps for linguistic data. In this case, we modelled the elastic net coefficients on the basis of the latitude and longitude values associated with the places in the profiles of the Twitter users, in addition to the interaction of the two axes ("tensor smooth", $te$). This gives rise to the following equation:
...
In our case, we want to know how the language users who are thought to be influential in our network behave. Are they are the forefront of innovation, or are they conservative? Depending on what profile we find, we might be able to discern in what phase of diffusion the *gij bent* innovation is situated.
