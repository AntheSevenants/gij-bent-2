...
# TODO volgorde legende is fucked
...
df <- data.frame(time, s_curve, bell_curve)

# Define cutoffs for adopter categories
mean_time <- 10
sd_time <- 2.5
cuts <- data.frame(
  x = c(mean_time - 2*sd_time, mean_time - sd_time, mean_time,
        mean_time + sd_time, mean_time + 2*sd_time),
  label = c("Innovators", "Early adopters", "Early majority", 
            "Late majority", "Laggards"),
  col = c("red", "orange", "green3", "darkgreen", "blue")
)
...
ggplot(df, aes(x = time)) +
  ...
  geom_vline(data = cuts, aes(xintercept = x, colour = label), linetype = "dashed") +
  scale_colour_manual(
    name = "Legend",
    values = c("S-curve (cumulative adoption)" = "purple",
               ...
               "Early majority" = "goldenrod3",
               "Late majority" = "darkgreen",
               "Laggards" = "blue")
  ) +
  labs(
    title = "S-curve and bell curve",
    x = "Time step",
